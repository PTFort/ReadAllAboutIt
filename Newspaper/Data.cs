...
namespace Newspaper
{
    public class Data
    {
        ...
        public static string[] templates = new string[]
        {
			...
			"Traffic Engineer Gives <ADJECTIVE-0-CAP> Presentation = In a hall packed with <ADJECTIVE> <CITIZEN-X-PLURAL> and mayors from other cities, David Rushkey gave <ADJECTIVE-0-INDEF> presentation, which captivated his audience with the details of traffic planning. Talking <ADVERB>, Mr. Rushkey explained how traffic in his home town of <CITY> is organized into a three-level hierarchy. Mayor <LASTNAME> of <CITY-0> <SAID>, \"This changes everything. I can't wait to return to <ADJECTIVE> <CITY-0> and <ADVERB> evict some <CITIZEN-X-PLURAL> to build more highways!\" One <CITIZEN> dissented, \"I've heard that roundabouts are the best. Why don't we <ADVERB> build more of those?\"", 
			...
			"Feelings of <EMOTION-0-CAP> for Green Energy = New construction projects have been <ADVERB> proposed in city council, focusing on the <ADJECTIVE> promise of green energy. One councilwoman <SAID>, \"I had <ADJECTIVE-X-INDEF> idea while <VERB-X-ING> my <NOUN>. Let's harness the power of the wind and the water to power our <ADJECTIVE> city.\" While this may be good for the environment, others <ADVERB> disagree. \"I have feelings of fear and <EMOTION> about these proposals. I mean, how do dams actually work? Is it going to <ADVERB> flood <CITYNAME>?\"",
			"Award Corruption? = <ADJECTIVE-X-CAP> allegations have arisen about the decision process for the regional \"Most Attractive City\" award. <FIRSTNAME> <LASTNAME>, our anonymous whistleblower, <SAID> \"Over the past few years, it's been <ADJECTIVE> cities that have won this award, not attractive ones. Have you seen those <BAD> spaghetti junctions in that last winner? <EXCLAM>\" Officials in the <REGION> have started <ADJECTIVE-X-INDEF> investigation into the claims, while Mayor <MAYORNAME>'s staff has started to coincidentally act very suspicious lately.",
			"New Fees for City Services? = <ADJECTIVE-X-CAP> rumours are swirling <ADVERB> in neighbouring cities about possible new fees. A new bill in the <REGION> allows each muncipality to start charging <CITIZEN-X-PLURAL> for previously free services. Already, <CITY> has begin charging <INT> <MONEY> for allowing <CITIZEN-X-PLURAL> to build their house in the new <ADJECTIVE-X-CAP> architectural style, which has some <CITIZEN-X-PLURAL> up-in-arms. Closer to home, <ADJECTIVE> city councillors in <CITYNAME> are deciding if they should <ADVERB> jump on the added revenue.",
			"Water Testing Begins = <CITYNAME> scientists have begun testing waterways around the city. One <SAID>, \"Mayor <MAYORNAME> is just <ADVERB> dumping wastewater in our <ADJECTIVE> water. You can't even <ADVERB> swim in the water because it's so <BAD>. <EXCLAM>\" Early research has predicted a rise in <DISEASE> from <CITIZEN-X-PLURAL> <ADVERB> drinking the water, and <CITIZEN-X-PLURAL> are advised to buy bottled water before attempting to <VERB>.",
		...
		public static string[] otherArticles = new string[] { "Spotlight on Traffic: pg. <INT>", "Driving Advice: pg. <INT>", "How's Your Driving?: pg. <INT>", "<CITYNAME>'s Best Restaurants: pg. <INT>", "Spotlight on <DISTRICT>", "<CITYNAME> Budget Issues: pg. <INT>", "Meet a <CITIZEN-X-CAP>: pg. <INT>", "Unrest on <COUNTRY>: pg. <INT>", "Profile Inside: New Art Trend",
			"Killer <ANIMAL-X-PLURAL-CAP> in <CITYNAME>?: pg. <INT>", "<DISEASE-X-CAP> Spreading in <CITYNAME>?: pg. <INT>", "<CITYNAME> Honours <PROFESSION-X-PLURAL-CAP>: pg. <INT>" ,"<PROFESSION-X-CAP> <VERB-X-PRESENT-CAP> <PROFESSION-X-CAP>: pg. <INT>" };
        ...
        public static string[] cities = new string[] { "Paris", "Tokyo", "San Francisco", "Los Angeles", "Stockholm", "Darwin", "Victoria" };
        public static string[] places = new string[] { "Mesopotamia", "Atlantis", "Afganistan", "Paraguay", "Egypt", "South Africa", "Brazil", "the U.S.S.R.", "Australia" };
